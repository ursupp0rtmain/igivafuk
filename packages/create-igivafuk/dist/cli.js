...
function isTextFile(filePath) {
  ...
  if (basename === ".gitkeep" || basename === ".gitignore" || basename === "_gitignore" || basename === ".editorconfig") {
    return true;
  }
...
Examples:
  ...
  npx ${PACKAGE_NAME} api -l go

Learn more: ${WEBSITE_URL}
`);
}
async function runCreate(argv = process.argv) {
...
${formatLanguagePresetList()}`);
  ...
  Se(`Done! cd ${projectName} and start building something good.
...
async function runDoctor(argv = process.argv) {
...
Fix it: npx ${PACKAGE_NAME} <name> for a fresh scaffold, or add missing files manually.`);
  ...
  Se(`Looking good. Now build something worth giving a f*** about.`);
}
...
async function runCli(argv = process.argv) {
  ...
  if (command === "create") {
    ...
    await runCreate2(["node", "create-igivafuk", ...rest]);
    return;
  }
  if (command === "doctor") {
    const { runDoctor: runDoctor2 } = await Promise.resolve().then(() => (init_doctor(), doctor_exports));
    await runDoctor2(["node", "igivafuk", "doctor", ...rest]);
    return;
  }
